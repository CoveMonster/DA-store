...
class Transformer(object):
    ...
    def __init__(self, config, wordEmbedding):
        ...
        with tf.name_scope("embedding"):
            ...
            # [btach_size , sequence_length, embedding_size + sequenceLen]
        ...
        with tf.name_scope("transformer"):
            for i in range(config.model.numBlocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    # 维度[batch_size, sequence_length,  embedding_size + sequenceLen]
            ...
            #outputs = tf.reshape(self.embeddedWords, [self.config.batchSize, config.sequenceLength])
        ...
        with tf.name_scope("loss"):
            ...
            if config.numClasses == 1:
                losses = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=tf.cast(tf.reshape(self.inputY, [-1, 1]), 
                 ...
                 losses = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits, labels=self.inputY)
